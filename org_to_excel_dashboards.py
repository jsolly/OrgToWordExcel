...
if __name__ == "__main__":
    ...
    for FOLDER_INDEX, FOLDER in enumerate(FOLDERS):
        ...
        FOLDER_ITEMS = get_items_from_folder(gis_obj=REGRESSION_DEVEXT_DBQA_GIS, folder=FOLDER, item_types=["Dashboard"])
        for ITEM in FOLDER_ITEMS:
            try:
                if ITEM.get_data() is not None:
                    ITEMS_WITH_FOLDER_AND_VERSION = (ITEM, FOLDER['title'], ITEM.get_data()['version']) 
            except Exception as e:
                print (e)
        ALL_ITEMS.append((ITEMS_WITH_FOLDER_AND_VERSION))

    for ITEM_INDEX, row in enumerate(WS.iter_rows(min_row=2, max_row=len(ALL_ITEMS)+1)):
