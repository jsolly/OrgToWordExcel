...
def add_hyperlink(paragraph, url, text, color, underline):
    ...
    # Create a new w:r_pr element
    r_pr = docx.oxml.shared.OxmlElement("w:r_pr")
...
if __name__ == "__main__":
    ...
    section = word_document.sections[
        0
    ]  # A section is a page. If you set a setting for the first page, it applies to all the other pages.
    section.orientation = (
        docx.enum.section.WD_ORIENT.LANDSCAPE
    )  # Make the pages landscape so we have more room
    (
        section.top_margin,
    ...
    )  # Get all of your AGOL folders

    for FOLDER in FOLDERS:
        word_document.add_heading(
            f"{FOLDER['title']}", level=1
        )  # Set a heading with the title of the current folder

        ITEMS = get_items_from_folder(
            gis_obj=REGRESSION_DEVEXT_DBQA_GIS, folder=FOLDER
        )  # Get all Items from the current AGOL fodler

        table = word_document.add_table(rows=1, cols=6, style="Light Grid")
        first_row_cells = table.rows[
            0
        ].cells  # Define the first row which will be the coloumn names
        ...
        for INDEX, ITEM in enumerate(ITEMS):
            ...
            item_details_link = (
                f"{REGRESSION_DEVEXT_DBQA_GIS._url}/home/item.html?id={ITEM.id}"
            )
            add_hyperlink(
                row_cells[3].paragraphs[0],
                item_details_link,
                "Item Details Page",
                "0000EE",
                True,
    ...
    word_document.save(
        f"{REGRESSION_DEVEXT_DBQA_GIS.users.me.username}.docx"
    )  # Save the document as username.docx
