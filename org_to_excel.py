...
if __name__ == "__main__":
    ...
    WS = WB.active
    FOLDERS = REGRESSION_DEVEXT_DBQA_GIS.users.me.folders # Get a list of all folders in AGOL
    print(f"I have {len(FOLDERS)} folders to work on")

    ALL_ITEMS = []
    for FOLDER_INDEX, FOLDER in enumerate(FOLDERS):
        print(f"I am working on Folder {FOLDER_INDEX} with name, {FOLDER['title']}")
        FOLDER_ITEMS = get_items_from_folder(gis_obj=REGRESSION_DEVEXT_DBQA_GIS, folder=FOLDER)
    ...
    for ITEM_INDEX, row in enumerate(WS.iter_rows(min_row=2, max_row=len(ALL_ITEMS)+1)): # start at the second row and keep adding rows until you run out of items
        ...
        row[4].value = f"""=HYPERLINK("{REGRESSION_DEVEXT_DBQA_GIS.url}//home/item.html?id={ALL_ITEMS[ITEM_INDEX][0].id}", "Open A copy in Develop 4")""" # Change to item details page
